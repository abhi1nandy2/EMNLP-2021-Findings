...
import random
from transformers import BertForTokenClassification, AdamW, BertConfig, RobertaConfig, RobertaForTokenClassification
...
# parser.add_argument("--mz_dir", type = str, required = True, help = "directory with file relating the section ids, sentences and labels")
...
mz_dir = "../../Sam_techqa_baselines/data/s10_mz_wikiqa_" + args.save_dir.split("/")[-1]
...
for stage in ['train', 'valid', 'test']:
	...
	with open(os.path.join(args.q_folderpath, stage + "_Q_A.json"), 'r') as f:
		stage_q_dict = json.load(f)
	df = get_split_for_rc_tokwise(stage, relation_df, stage_q_dict, corpus_dict)
	df_list.append(df)
...
train_dataset = get_dataset_tokwise(df_train, tokenizer)
val_dataset = get_dataset_tokwise(df_valid, tokenizer)
prediction_dataset = get_dataset_tokwise(df_test, tokenizer)
...
if args.checkpoint_path == "":
	...
	if "more_grad" in args.checkpoint_path:
		config = RobertaConfig(max_position_embeddings=514, vocab_size=50265, type_vocab_size=1, num_labels = 4, output_attentions = False, output_hidden_states = False)
